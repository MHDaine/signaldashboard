...
@app.get("/api/signals")
def get_signals(min_score: int = 0):
...
def unapprove_signal(signal_id: str):
    data = _load_signals_file()
    for sig in data.get("signals", []):
        if sig.get("id") == signal_id:
            sig.pop("approved", None)
...
@app.put("/api/sources")
def save_sources(cfg: SourcesConfig):
    _save_sources({
...
def collect_and_rank(req: CollectRequest):
...
@app.post("/api/enrich")
def enrich_signals(req: EnrichRequest):
